...
@ManagedBean(name = "nonemployeeAssignmentReport")
@ViewScoped
public class NonemployeeAssignmentReport extends BaseBacking implements Serializable {
	 ...
	 * Default Constructor
	 */
	public NonemployeeAssignmentReport() {
		...
		try {
			...
			this.setServerSideErrorMessages(getParameterizedMessage(e.getMessage(), e.getParams()));
			   ...
			   Log4j.traceErrorException(NonemployeeAssignmentReport.class, e, "NonemployeeAssignmentReport");
	...
	public void printReport() {
		...
		try {
			...
			byte[] bytes = AssignmentNonEmployeeService.getNonemployeeAssignmentReportBytes(HijriDateService.getHijriDateString(startDate), HijriDateService.getHijriDateString(endDate), regionId, regionName, sectorId, officerId == null ? FlagsEnum.ALL.getCode() : officerId, active, assignmentStatus , loginEmpData.getFullName(), assignmentType);
			super.print(bytes, "Nonemployee_Assignment_Report");
		} catch (BusinessException e) {
			this.setServerSideErrorMessages(getParameterizedMessage(e.getMessage(), e.getParams()));
		}catch (Exception e) {
			   Log4j.traceErrorException(NonemployeeAssignmentReport.class, e, "NonemployeeAssignmentReport");
	...
	public void printStatisticalReport() {
		try {
			...
			byte[] bytes = AssignmentNonEmployeeService.getStatisticalNonemployeeAssignmentReportBytes(HijriDateService.getHijriDateString(startDate), HijriDateService.getHijriDateString(endDate), regionId, regionName, sectorId, officerId == null ? FlagsEnum.ALL.getCode() : officerId, active, assignmentStatus , loginEmpData.getFullName(), assignmentType);
			super.print(bytes, "Statistical_Nonemployee_Assignment_Report");
		} catch (BusinessException e) {
			this.setServerSideErrorMessages(getParameterizedMessage(e.getMessage(), e.getParams()));
		}catch (Exception e) {
			   Log4j.traceErrorException(NonemployeeAssignmentReport.class, e, "NonemployeeAssignmentReport");
