...
public class SecurityCheckRegionWorkFlow extends BaseWorkFlow {
	private SecurityCheckRegionWorkFlow() {
	}
	...
	public static void initSecurityCheck(SecurityCheckData securityCheck, List<SecurityCheckEmployeeData> employeeDataList, List<SecurityCheckEmployeeData> employeeDataDeletedList, List<SecurityCheckNonEmployeeData> nonEmployeeDataList, List<SecurityCheckNonEmployeeData> nonEmployeeDataDeletedList, EmployeeData loginUser, String attachments, WFTask dmTask, CustomSession... useSession) throws BusinessException {
		...
		try {
			...
			if (e instanceof BusinessException) {
				...
				Log4j.traceErrorException(SecurityCheckRegionWorkFlow.class, e, "SecurityCheckRegionWorkFlow");
		...
		try {
			...
			if (dmTask.getAssigneeWfRole().equals(WFTaskRolesEnum.REGION_INTELLIGENCE_MANAGER.getCode())) {
				...
				if (securityCheck.getRequestSource().equals(RequestSourceEnum.REGION_DIRECTOR.getCode()) || securityCheck.getRequestSource().equals(RequestSourceEnum.OTHERS.getCode())) {
					...
					Long originalId = DepartmentService.getDepartmentManager(unitId);
					Long assigneeId = getDelegate(originalId, WFProcessesEnum.SECURITY_CHECK.getCode());
					completeWFTask(dmTask, WFTaskActionsEnum.APPROVE.getCode(), new Date(), currHijriDate, instance.getInstanceId(), assigneeId, originalId, WFTaskUrlEnum.SECURITY_CHECK.getCode(), WFTaskRolesEnum.SECURITY_CHECK_GENERAL_INTELLIGENCE_MANAGER.getCode(), "1", arabicDetailsSummary, englishDetailsSummary, session);
				}

			}

			else if (dmTask.getAssigneeWfRole().equals(WFTaskRolesEnum.SECURITY_CHECK_GENERAL_INTELLIGENCE_MANAGER.getCode())) {
				Long unitId = WFPositionService.getWFPosition(WFPositionsEnum.GENERAL_INTELLIGENCE_DIRECTORATE.getCode()).getUnitId();
				Long originalId = DepartmentService.getDepartmentManager(unitId);
				Long assigneeId = getDelegate(originalId, WFProcessesEnum.SECURITY_CHECK.getCode());
				completeWFTask(dmTask, WFTaskActionsEnum.APPROVE.getCode(), new Date(), currHijriDate, instance.getInstanceId(), assigneeId, originalId, WFTaskUrlEnum.SECURITY_CHECK.getCode(), WFTaskRolesEnum.SECURITY_CHECK_GENERAL_DIRECTOR.getCode(), "1", arabicDetailsSummary, englishDetailsSummary, session);
			}

			else if (dmTask.getAssigneeWfRole().equals(WFTaskRolesEnum.SECURITY_CHECK_GENERAL_DIRECTOR.getCode())) {
				// TODO processing
