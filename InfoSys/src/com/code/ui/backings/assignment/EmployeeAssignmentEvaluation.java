...
@ManagedBean(name = "employeeAssignmentEvaluation")
@ViewScoped
public class EmployeeAssignmentEvaluation extends WFBaseBacking implements Serializable {
	...
	private boolean ended; // Approved End data before today

	/**
	 * Default Constructor and Initializer
	 */
	public EmployeeAssignmentEvaluation() {
		super();
		super.init();
		this.init();
		try {
			...
			if (currentTask == null) {
				...
				if (assignmentEvaluation.getId() != null) {
					...
					assignmentEvaluation.setTotalEvaluation(0.0);
				}
				...
				if (assignmentEvaluation.getEvaluationAction().equals(AssignmentEvaluationActionEnum.CONTINUE.getCode()) || assignmentEvaluation.getEvaluationAction().equals(AssignmentEvaluationActionEnum.EXTEND.getCode())) {
					screenTitle = getParameterizedMessage("label_assignmentContinueApprove");
				} else if (assignmentEvaluation.getEvaluationAction().equals(AssignmentEvaluationActionEnum.END.getCode())) {
					screenTitle = getParameterizedMessage("label_assignmentEndApprove");
				}
				if (assignmentEvaluation.getReasons() != null && !assignmentEvaluation.getReasons().isEmpty()) {
					String[] reasons = assignmentEvaluation.getReasons().split(",");
					assignmentEvaluation.setAssignmentEvalReasonsList(Arrays.asList(reasons));
			   ...
			   Log4j.traceErrorException(EmployeeAssignmentEvaluation.class, e, "EmployeeAssignmentEvaluation");
			   this.setServerSideErrorMessages(getParameterizedMessage("error_general"));
		  }
	}
	...
	public void getInfoDetails() {
		try {
			employeeRelatedInfoList = InfoService.getInfoDataRelatedEntity(FlagsEnum.ALL.getCode(), FlagsEnum.ALL.getCode(), FlagsEnum.ALL.getCode(), FlagsEnum.ALL.getCode(), assignmentDetailData.getEmployeeId(), null, loginEmpData);
		} catch (BusinessException e) {
			this.setServerSideErrorMessages(getParameterizedMessage(e.getMessage(), e.getParams()));
		}
	}
	...
	public void getAssignDetails() {
		try {
			previousAssignmentsList = AssignmentService.getAssignmentPrevDetailData(assignmentDetailData.getEmployeeId(), null, assignmentDetailData.getStartDate());
		} catch (BusinessException e) {
			this.setServerSideErrorMessages(getParameterizedMessage(e.getMessage(), e.getParams()));
		}
	}
	...
	public String saveAndSend() {
		try {
			...
			if (ended) {
				...
				AssignmentEmployeeService.updateEvaluation(assignmentEvaluation, assignmentEvaluationPointDataList, loginEmpData);
			} else {
				...
				AssignmentEmployeeService.addAssignemntEvaluation(assignmentEvaluation, assignmentEvaluationPointDataList, loginEmpData);
			}
		  } catch (BusinessException e) {
			   this.setServerSideErrorMessages(getParameterizedMessage(e.getMessage(), e.getParams()));
			   return null;
	...
	public String doApprove() {
		try {
			// currentTask.setNotes(assignmentEvaluation.getRemarks());
			if (role.equals(WFTaskRolesEnum.HQ_SECURITY_AFFAIRS_APPROVER_MANAGER.getCode())) {
				EmployeesAssignementEvaluationWorkFlow.approveHQManager(currentTask, assignmentEvaluation, null);
				...
				assignmentEvaluation.setStatus(AssignmentStatusEnum.APPROVED.getCode());
				EmployeesAssignementEvaluationWorkFlow.approveHQGeneralManager(currentTask, assignmentEvaluation, assignmentDetailData, null);
			} else if (role.equals(WFTaskRolesEnum.REGION_INTELLIGENCE_MANAGER.getCode())) {
				EmployeesAssignementEvaluationWorkFlow.approveRegionManager(currentTask, assignmentEvaluation, loginEmpData, null);
			} else if (role.equals(WFTaskRolesEnum.REGION_SECURITY_INTELLIGENCE_MANAGER.getCode())) {
				EmployeesAssignementEvaluationWorkFlow.approveRegionSecurityManager(currentTask, assignmentEvaluation, null);
			}
		  } catch (BusinessException e) {
			   this.setServerSideErrorMessages(getParameterizedMessage(e.getMessage(), e.getParams()));
			   return null;
	...
	public String doNotified() {
		try {
			EmployeesAssignementEvaluationWorkFlow.doNotify(currentTask);
		} catch (BusinessException e) {
			this.setServerSideErrorMessages(getParameterizedMessage(e.getMessage(), e.getParams()));
			return null;
		}
	...
	public String doReject() {
		try {
			...
			EmployeesAssignementEvaluationWorkFlow.rejectEmployeeAssignmentEvaluation(currentTask, assignmentEvaluation, null, loginEmpData);
		} catch (BusinessException e) {
			this.setServerSideErrorMessages(getParameterizedMessage(e.getMessage(), e.getParams()));
			return null;
		}
	...
	public void print() {
		try {
			if (assignmentEvaluation.getEvaluationAction().equals(AssignmentEvaluationActionEnum.END.getCode())) {
				byte[] bytes = AssignmentService.getEmployeeEvalEndDetailBytes(assignmentDetailData.getAssginmentId(), assignmentDetailData.getId(), assignmentEvaluation.getId(), assignmentEvaluation.getReasons(), loginEmpData.getFullName());
				super.print(bytes, "employeeEvalEndDetail");
			} else {
				byte[] bytes = AssignmentService.getEmployeeEvalResumeDetailBytes(assignmentDetailData.getAssginmentId(), assignmentDetailData.getId(), assignmentEvaluation.getId(), assignmentEvaluation.getExtendPeriod(), assignmentEvaluation.getReasons(), loginEmpData.getFullName());
				super.print(bytes, "employeeEvalResumeDetail");
			}
		} catch (BusinessException e) {
			this.setServerSideErrorMessages(getParameterizedMessage(e.getMessage(), e.getParams()));
		}
	...
	/**
	 * Re calculate percentage average
	 * 
	 * @param e
	 */
	public void percentChanged(AjaxBehaviorEvent e) {
		Double sum = 0.0;
		for (AssignmentEvaluationPointData point : assignmentEvaluationPointDataList) {
			sum += (double) +point.getPercent();
		}
		DecimalFormat df = new DecimalFormat("##.##");
		assignmentEvaluation.setTotalEvaluation(Double.parseDouble((df.format(sum / assignmentEvaluationPointDataList.size()))));
	}
