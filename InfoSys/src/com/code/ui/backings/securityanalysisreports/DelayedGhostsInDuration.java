...
@ManagedBean(name = "delayedGhostsInDuration")
@ViewScoped
public class DelayedGhostsInDuration extends BaseBacking implements Serializable {
	 ...
	 * Default Constructor
	 */
	public DelayedGhostsInDuration() {
		try {
	...
	public void print() {
		...
		try {
			...
			byte[] bytes = FollowUpService.getDelayedGhostInDurationReportBytes(regionId, regionName, caseTypeId, caseTypeDesc, networkId, networkName, ghostName, HijriDateService.getHijriDateString(fromDate), HijriDateService.getHijriDateString(toDate), loginEmpData.getFullName());
			super.print(bytes, "Delayed Ghosts In Duration Report");
		} catch (BusinessException e) {
			this.setServerSideErrorMessages(getParameterizedMessage(e.getMessage(), e.getParams()));
		}catch (Exception e) {
			   Log4j.traceErrorException(DelayedGhostsInDuration.class, e, "DelayedGhostsInDuration");
