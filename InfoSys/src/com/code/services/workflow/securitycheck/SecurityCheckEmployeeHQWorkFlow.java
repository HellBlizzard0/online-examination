...
public class SecurityCheckEmployeeHQWorkFlow extends BaseWorkFlow {
	private SecurityCheckEmployeeHQWorkFlow() {
	}
	...
	public static void initSecurityCheck(SecurityCheckData securityCheck, List<SecurityCheckEmployeeData> employeeDataList, List<SecurityCheckEmployeeData> employeeDataDeletedList, List<SecurityCheckNonEmployeeData> nonEmployeeDataList, List<SecurityCheckNonEmployeeData> nonEmployeeDataDeletedList, EmployeeData loginUser, String attachments, WFTask dmTask, CustomSession... useSession) throws BusinessException {
		...
		try {
			...
			if (regionId != null) {
				...
				if (dmTask == null) {
					addWFTask(securityCheck.getwFInstanceId(), assigneeId, originalId, new Date(), hijriCurDate, WFTaskUrlEnum.SECURITY_CHECK.getCode(), WFTaskRolesEnum.GENERAL_INTELLIGENCE_MANAGER.getCode(), "1", arabicDetailsSummary, englishDetailsSummary, session);
				} else {
					completeWFTask(dmTask, WFTaskActionsEnum.SEND.getCode(), new Date(), hijriCurDate, securityCheck.getwFInstanceId(), assigneeId, originalId, WFTaskUrlEnum.SECURITY_CHECK.getCode(), WFTaskRolesEnum.GENERAL_INTELLIGENCE_MANAGER.getCode(), "1", arabicDetailsSummary, englishDetailsSummary, session);
				}
			}
			if (!isOpenedSession)
			...
			if (e instanceof BusinessException) {
				...
				Log4j.traceErrorException(SecurityCheckEmployeeHQWorkFlow.class, e, "SecurityCheckHQWorkFlow");
	...
	/**
	 * doApproveRejectRegionIntelligence
	 * 
	 * @param dmTask
	 * @param action
	 * @param securityCheck
	 ...
	 * @throws BusinessException
	 */
	public static void doApproveRejectRegionSecurity(WFTask dmTask, WFTaskActionsEnum action, SecurityCheck securityCheck, EmployeeData loginUser, String attachments, CustomSession... useSession) throws BusinessException {
		boolean isOpenedSession = isSessionOpened(useSession);
		CustomSession session = isOpenedSession ? useSession[0] : DataAccess.getSession();
		try {
			if (!isOpenedSession)
	...
	public static void doProcessing(WFTask dmTask, SecurityCheck securityCheck, List<SecurityCheckEmployeeData> employeeDataList, List<SecurityCheckNonEmployeeData> nonEmployeeDataList, EmployeeData loginUser, String attachments, CustomSession... useSession) throws BusinessException {
		...
		try {
			...
			if (regionId != null) {
				...
				Long originalId = DepartmentService.getDepartmentManager(unitId);
				Long assigneeId = getDelegate(originalId, WFProcessesEnum.SECURITY_CHECK.getCode());
				completeWFTask(dmTask, WFTaskActionsEnum.APPROVE.getCode(), new Date(), hijriCurDate, instance.getInstanceId(), assigneeId, originalId, WFTaskUrlEnum.SECURITY_CHECK.getCode(), WFTaskRolesEnum.GENERAL_INTELLIGENCE_MANAGER_PROCESSING.getCode(), "1", arabicDetailsSummary, englishDetailsSummary, session);
			}
	...
	/**
	 * doApproveRejectProcessingRegionSecurityManager
	 * 
	 * @param dmTask
	 * @param action
	 * @param securityCheck
	 ...
	 * @throws BusinessException
	 */
	public static void doApproveRejectProcessingRegionSecurityManager(WFTask dmTask, WFTaskActionsEnum action, SecurityCheck securityCheck, EmployeeData loginUser, String attachments, CustomSession... useSession) throws BusinessException {
		boolean isOpenedSession = isSessionOpened(useSession);
		CustomSession session = isOpenedSession ? useSession[0] : DataAccess.getSession();
		try {
			if (!isOpenedSession)
			...
			if (action.equals(WFTaskActionsEnum.REJECT)) {
				...
				Long originalId = DepartmentService.getDepartmentManager(regionId);
				Long assigneeId = getDelegate(originalId, WFProcessesEnum.SECURITY_CHECK.getCode());
				completeWFTask(dmTask, WFTaskActionsEnum.APPROVE.getCode(), new Date(), currHijriDate, instance.getInstanceId(), assigneeId, originalId, WFTaskUrlEnum.SECURITY_CHECK.getCode(), WFTaskRolesEnum.REGION_DIRECTOR_PROCESSING.getCode(), "1", arabicDetailsSummary, englishDetailsSummary, session);
			}
			if (!isOpenedSession)
				session.commitTransaction();
		} catch (Exception e) {
